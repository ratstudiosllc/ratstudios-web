...
type SessionStatus = "queued" | "running" | "completed" | "failed" | "canceled" | "retrying" | string;
...
function classifySession(session: OpenClawSession): SessionStatus {
...
async function getAdminSnapshot(): Promise<{ generatedAt: string; metricCards: MetricCard[]; notes: string[] }> {
  ...
  try {
    ...
    const sessionsToday = sessions.filter((session) => {
      ...
      return isSameLocalDay(new Date(session.updatedAt), today);
    });
    ...
    const metricCards: MetricCard[] = [
      {
        ...
        note: `${Math.max(0, sessionsToday.length - failedRuns)} completed, ${failedRuns} failed today.`,
        availabilityTone: "bg-emerald-100 text-emerald-800",
        availabilityLabel: "Live",
        sourceTone: "bg-violet-100 text-violet-800",
      ...
      },
      {
        ...
        note: `${status.tasks?.queued ?? 0} queued right now.`,
        availabilityTone: "bg-emerald-100 text-emerald-800",
        availabilityLabel: "Live",
        sourceTone: "bg-violet-100 text-violet-800",
        sourceLabel: "Runtime snapshot",
      ...
      },
      {
        ...
        note: failedRuns ? `${failedRuns} task${failedRuns === 1 ? "" : "s"} failed today.` : "No failed tasks detected today.",
        availabilityTone: "bg-emerald-100 text-emerald-800",
        availabilityLabel: "Live",
        sourceTone: "bg-violet-100 text-violet-800",
        sourceLabel: "Runtime snapshot",
      ...
      },
      {
        ...
        value: estimatedCost,
        note: estimatedCost === "Unavailable" ? "Only shown when visible sessions include token totals." : "Derived from visible session token totals when present.",
        availabilityTone: estimatedCost === "Unavailable" ? "bg-neutral-100 text-neutral-700" : "bg-amber-100 text-amber-800",
        availabilityLabel: estimatedCost === "Unavailable" ? "Not wired" : "Inferred",
        sourceTone: "bg-violet-100 text-violet-800",
        sourceLabel: "Runtime snapshot",
        updatedAt: generatedAt,
      },
      {
        key: "modelAuthHealth",
        label: "Model auth health",
        value: "Unavailable",
        note: "No trustworthy auth-health summary is exposed to this app yet.",
        availabilityTone: "bg-neutral-100 text-neutral-700",
        availabilityLabel: "Not wired",
        sourceTone: "bg-neutral-100 text-neutral-700",
    ...
    return {
      ...
      notes: [
        ...
        estimatedCost === "Unavailable"
          ? "Visible sessions did not expose enough token data for an honest cost estimate."
          : "Cost is estimated from visible session token totals, so treat it as directional rather than billing-grade.",
      ],
    };
  } catch (error) {
    return {
      ...
      metricCards: [
        {
          key: "runsToday" as const,
          label: "Runs today",
          value: "Unavailable",
          note: "OpenClaw runtime could not be read from this deployment target.",
          availabilityTone: "bg-neutral-100 text-neutral-700",
          availabilityLabel: "Not wired",
          sourceTone: "bg-neutral-100 text-neutral-700",
          sourceLabel: "Unavailable",
        ...
        },
        {
          ...
          label: "Active runs",
          value: "Unavailable",
          note: "This environment cannot currently read local OpenClaw state.",
          availabilityTone: "bg-neutral-100 text-neutral-700",
          availabilityLabel: "Not wired",
          sourceTone: "bg-neutral-100 text-neutral-700",
          sourceLabel: "Unavailable",
        ...
        },
        {
          ...
          label: "Failed runs",
          value: "Unavailable",
          note: "No runtime snapshot was available.",
          availabilityTone: "bg-neutral-100 text-neutral-700",
          availabilityLabel: "Not wired",
          sourceTone: "bg-neutral-100 text-neutral-700",
          sourceLabel: "Unavailable",
        ...
        },
        {
          ...
          label: "Estimated cost",
          value: "Unavailable",
          note: "Cost stays hidden when runtime metrics are unavailable.",
          availabilityTone: "bg-neutral-100 text-neutral-700",
          availabilityLabel: "Not wired",
          sourceTone: "bg-neutral-100 text-neutral-700",
          sourceLabel: "Unavailable",
      ...
      <div className="mx-auto max-w-7xl px-6 py-10">
        ...
        <section className="mt-8 grid gap-4 md:grid-cols-2 xl:grid-cols-5">
          {data.metricCards.map((metric) => (
            <MetricCardView key={metric.key} metric={metric} />
          ))}
        </section>
        ...
        <section className="mt-8 grid gap-6 xl:grid-cols-2">
          <div className="rounded-[32px] border border-black/5 bg-white p-6 shadow-sm">
            ...
            <div className="mt-5 space-y-3 text-sm text-neutral-600">
              <div className="rounded-2xl bg-[#fcfaf7] p-4">
                <p className="font-semibold text-neutral-900">Task sessions</p>
                <p className="mt-1">Read server-side from OpenClaw session and status commands, not mocked browser data.</p>
              </div>
              <div className="rounded-2xl bg-[#fcfaf7] p-4">
                <p className="font-semibold text-neutral-900">Estimated cost</p>
                <p className="mt-1">Only shown when visible sessions expose token totals. Otherwise it stays unavailable on purpose.</p>
              </div>
              <div className="rounded-2xl bg-[#fcfaf7] p-4">
                <p className="font-semibold text-neutral-900">Model auth health</p>
                <p className="mt-1">Still omitted as a real metric until this app has a trustworthy server-side source.</p>
              </div>
            </div>
          </div>
          ...
          <div className="rounded-[32px] border border-black/5 bg-white p-6 shadow-sm">
            ...
            <div className="mt-5 space-y-3 text-sm text-neutral-600">
              {data.notes.map((note) => (
                <div key={note} className="rounded-2xl bg-[#fcfaf7] p-4">{note}</div>
              ))}
            </div>
          </div>
        </section>
